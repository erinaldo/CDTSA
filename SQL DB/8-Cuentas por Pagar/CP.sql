...
--select * from dbo.cppDocumentosCP 
-- exec  [fnica].[fafGetConsecutivosFaltantes]  '20150223', '20150223', 'ET01', 'R/C'

create Procedure [fnica].[fafGetConsecutivosFaltantes] @FechaInicial datetime, @FechaFinal datetime, @CodSucursal nvarchar(4), @IDClase nvarchar(20)
as
Declare @MinNumber INT, @MaxNumber INT, @AntMinimo int 
set nocount on
Select IDDocumentoCP, CAST (DOCUMENTO AS INT ) DOCUMENTO, 'Existe' Estado, Fecha, MontoOriginal ValorCO, 
case when  (not (TipoCambio is null) and TipoCambio <> 0) then MontoOriginal/TipoCambio else 0 end ValorDL, ConceptoUsuario, IDProveedor,
Anulado
INTO #Resultado
from dbo.cppDocumentosCP 
...
--SELECT @MinNumber MinNumber

if @AntMinimo > 0 
begin
	if not exists(Select IDDocumentoCP From dbo.cppDocumentosCP 
		where  CodSucursal = @CodSucursal AND TipoDocumento= 'C' and IDClase = @IDClase and IDDocumentoCP = @AntMinimo)
	insert #Faltan (Documento, Estado, Fecha, ValorCO, ValorDL, ConceptoUsuario, IDProveedor)
	values (@AntMinimo, '', '19800101', 0,0, '','')
end

IF ( SELECT COUNT(*) FROM dbo.cppDocumentosCP WHERE IDClase = 'R/C' AND CodSucursal = @CodSucursal AND DOCUMENTO = CAST(@AntMinimo AS NVARCHAR(20)) ) > 0
	DELETE FROM #Faltan


	
SELECT DOCUMENTO, Fecha , case when X.IDProveedor ='' then 'oops!' else  isnull(X.IDProveedor,'Falta') end IDProveedor,
isnull(rtrim(NombresCliente), 'Falta') 
+ ' ' + isnull(rtrim(ApellidosCliente),'Consecutivo') Nombre, ValorCO, ValorDL,
isnull(ConceptoUsuario,'') ConceptoUsuario, IDDocumentoCP, Anulado, C.IDCategoria  
INTO #ResultadoFinal
FROM 
(
Select Documento, Estado, Fecha,  ValorCO, ValorDL, ConceptoUsuario, IDProveedor, IDDocumentoCP, Anulado 
From #Resultado
union all
Select Documento, Estado, Fecha,  ValorCO, ValorDL, ConceptoUsuario, IDProveedor, -1 IDDocumentoCP, 0 Anulado 
From #Faltan
) X left join dbo.cppProveedores C on X.IDProveedor = C.IDProveedor
ORDER BY DOCUMENTO
 

declare @DocTemporal nvarchar(20)

if (select count(*) from #ResultadoFinal )= 1 
BEGIN
	select @DocTemporal = Documento From #ResultadoFinal

	if @DocTemporal is null
		Delete From #ResultadoFinal 
END
...
create Function dbo.cppMayRevertirAsiento (  @DocumentoCredito bit, @Factura nvarchar(10), @CodSucursal nvarchar(4), @IDDocumentoCP int )
returns bit 
as
begin
...
create procedure dbo.cppGetDocNoContabilzados @DetalleSoloFacturas bit = null
-- DEVUELVE LOS DOCUMENTOS NO CONTABILIZADOS,SE EXCLUYEN LOS INTERESES Y DESLIZAMIENTOS YA QUE ESTOS SE ONTABILIZAN ENN LOS R/C
as
set nocount on 
if @DetalleSoloFacturas is null
	set @DetalleSoloFacturas = 0
Select CODSUCURSAL, SUCURSAL , ultimodiacerrado 
into #Cierres
from dbo.globalsucursales
Where EsSucursal = 1
cREATE TABLE #Resultado ( CodSucursal nvarchar(10), Sucursal nvarchar(255), IDProveedor nvarchar(20),
UltimoDiaCerrado datetime, IDDocumentoCP int, IDClase nvarchar(10), documento nvarchar(20), Fecha datetime, 
MontoOriginal decimal (28,4), SaldoActual decimal (28,4), Tipo nvarchar(25), PaqueteFactura bit default 0 )

insert #Resultado ( CodSucursal, Sucursal, IDProveedor, UltimoDiaCerrado, IDDocumentoCP, IDClase, documento, Fecha,
MontoOriginal, SaldoActual, Tipo )
-- facturas de contado
SELECT D.CodSucursal, C.Sucursal, D.IDProveedor, C.UltimoDiaCerrado, 0 IDDocumentoCP, --CAST(FACTURA AS INT)  IDDocumentoCP,
'FAC' IDClase,FACTURA Documento,FECHAFACTURA Fecha, TotalFactura MontoOriginal, TotalFactura SaldoActual, 'CONTADO' TIPO 
FROM dbo.FAFFACTURA D INNER JOIN #Cierres C on D.CodSucursal = C.CodSucursal
where D.ANULADA = 0 and Asiento is null --and D.TIPO = '1'
and d.fechafactura <= C.ultimodiacerrado  and D.FechaFACTURA >'20150131' 

/*UNION ALL
-- DOCUMENTOSXCOBRAR
Select D.CodSucursal, C.Sucursal, D.IDProveedor, C.UltimoDiaCerrado, D.IDDocumentoCP, D.IDClase, D.Documento, D.Fecha, D.MontoOriginal, D.SaldoActual , 'CREDITO' TIPO 
From dbo.cppDocumentosCP D inner join #Cierres C on D.CodSucursal = C.CodSucursal
where Contabilizado =0 and Anulado = 0 and Asiento is null
and d.fecha <= C.ultimodiacerrado and D.Fecha >'20150131' and IDClase not in ('INT', 'D/C')
order by D.CodSucursal, D.IDClase, D.Fecha desc
*/
UPDATE R SET PaqueteFactura = 1
from #Resultado R INNER JOIN dbo.DOCUMENTO_INV I
ON dbo.invGetNoFactura(R.CodSucursal, R.documento) = I.DOCUMENTO_INV 
where R.IDClase = 'FAC'

if @DetalleSoloFacturas = 0
begin
	Select CodSucursal, IDProveedor, UltimoDiaCerrado, IDDocumentoCP, IDClase, documento, Fecha,
	MontoOriginal, SaldoActual, Tipo ,PaqueteFactura
	from #Resultado
end
if @DetalleSoloFacturas = 1
begin
	Select R.CodSucursal, R.IDProveedor, D.Factura, R.Fecha,  D.Articulo, A.Descripcion, D.Cantidad, D.PrecioUnitario,( D.PrecioUnitario * D.Cantidad) Subtotal,
	D.iva, ( D.PrecioUnitario * D.Cantidad)+D.iva Total, A.Articulo_Cuenta, C.DESCRIPCION DescrCuenta
	from #Resultado R inner join dbo.fafFacturaDetalle D
	on R.CodSucursal = D.CodSucursal and R.Documento = D.Factura 
	inner join dbo.Articulo A on D.Articulo = A.Articulo
	inner join dbo.ARTICULO_CUENTA C
	on A.ARTICULO_CUENTA = C.ARTICULO_CUENTA 
	order by R.CodSucursal, R.documento 	
end
...
go
-- A PARTI DE AQUI VA LO DE EDJAMIT


ALTER PROCEDURE [fnica].[cppGetRecibosByCriterio]
...
Select DOCUMENTO NUMERORECIBO, a.CODSUCURSAL,FECHA FECHAINGRESO,a.IDProveedor,NOMBRESCLIENTE+' '+APELLIDOSCLIENTE CLIENTE,isnull(RECIBIMOSDE,'') RECIBIMOSDE,
MONTOORIGINAL VALOR,(MONTOORIGINAL/(SELECT dbo.globalGetUltTasadeCambio (FECHA))) VALORDOLAR, Cast(ANULADO as int) ANULADA,
(SELECT dbo.globalGetUltTasadeCambio(FECHA)) TIPOCAMBIO,IDDocumentoCP    
...
AND (TipoDocumento='C' and IDCLASE='R/C')


--SELECT    NUMERORECIBO, CODSUCURSAL, FECHAINGRESO,IDProveedor,CLIENTE,RECIBIMOSDE,
-- VALOR, VALORDOLAR, ANULADA,TIPOCAMBIO               
--FROM         dbo.cpprecibos
